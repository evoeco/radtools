...
sub merge
{
	...
	for (my $i = 0; $i < $len; $i++)
	{
		...
		if ($nuc_a eq $nuc_b)
		{ $seq .= $nuc_a; }
		elsif (($nuc_a eq 'A' and $nuc_b eq 'T') or ($nuc_b eq 'A' and $nuc_a eq 'T'))
		{ $seq .= 'W'; }
		elsif (($nuc_a eq 'G' and $nuc_b eq 'T') or ($nuc_b eq 'G' and $nuc_a eq 'T'))
		{ $seq .= 'K'; }
		elsif (($nuc_a eq 'A' and $nuc_b eq 'C') or ($nuc_b eq 'A' and $nuc_a eq 'C'))
		{ $seq .= 'M'; }
		elsif (($nuc_a eq 'A' and $nuc_b eq 'G') or ($nuc_b eq 'A' and $nuc_a eq 'G'))
		{ $seq .= 'R'; }
		elsif (($nuc_a eq 'C' and $nuc_b eq 'T') or ($nuc_b eq 'C' and $nuc_a eq 'T'))
		{ $seq .= 'Y'; }
		elsif (($nuc_a eq 'C' and $nuc_b eq 'G') or ($nuc_b eq 'C' and $nuc_a eq 'G'))
		{ $seq .= 'S'; }
		else { $seq .= 'N'; }
	}
	return $seq;
...
while (<>)
{
	...
	if (@row > 2)
	{
		...
		{
			$name = $row[0];
			if ($subset and !grep($_ eq $name, @outgs))
			{ next; }
			@prev_row = @row;
			@{$nodata{$name}} = ();
